...
get your own API Key to run the examples below. 
...
```{r}
pres_list <-list_guid(url_base, api_key)
pres_lit[3]
```
...
has already been analysed and summarized. It is not immediately obvious which
...
There are two utilities to help cleaning the currency data: `clean_currency` y
`get_currency_symbol`.  For example:
